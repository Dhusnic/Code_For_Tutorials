...
- [config.worker0.yml](./config.worker0.yml) and [config.worker1.yml](./config.worker1.yml): worker-specific Go runtime configs.
...
```powershell
.\bin\rca-engine.exe --config .\config.worker0.yml --run-once
```
