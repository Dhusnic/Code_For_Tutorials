...
from vector_functions import Model_function
    ...
    def chat_page(self,chat_id):
        ...
        if prompt:
            ...
            if os.path.exists(f"./persist"):
                retriever = self.model_function.load_retriever(collection_name=collection_name)
            else:
                retriever = None

            # Ask question using the retriever
            response = (
                self.model_function.generate_answer_from_context(retriever, prompt)
                if retriever
            ...
            # Display AI response
            with st.chat_message("assistant"):
                st.write_stream(self.stream_response(response))
        ...
        with st.sidebar:
            ...
            if "prev_temp" not in st.session_state:
                st.session_state.prev_temp = 0.3  # or default value

            self.model_function.temprature = st.slider(
                "temprature of the model",
                min_value=0.0,
                max_value=1.0,
                value=0.3,  # default
                step=0.01
            ...
            if self.model_function.temprature != st.session_state.prev_temp:
                st.session_state.prev_temp = self.model_function.temprature
                self.model_function.change_in_model()
            ...
            if uploaded_file:
                ...
                with st.spinner("Processing document..."):
                    ...
                    # Load document
                    document = self.model_function.load_document(temp_file_path)

                    # Create or update collection for this chat
                    collection_name = f"chat_{chat_id}"
                    if not os.path.exists(f"./persist/{collection_name}"):
                        vectordb = self.model_function.create_collection(collection_name, document)
                    else:
                        vectordb = self.model_function.load_collection(collection_name)
                        vectordb = self.model_function.add_documents_to_collection(vectordb, document)
                    ...
                    with st.spinner("Processing link..."):
                        ...
                        try:
                            ...
                            if not os.path.exists(f"./persist"):
                                self.model_function.create_collection(collection_name, documents)
                            else:
                                vectordb = self.model_function.load_collection(collection_name)
                                self.model_function.add_documents_to_collection(vectordb, documents)
